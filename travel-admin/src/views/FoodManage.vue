    ...
    <table class="table table-hover food-table">
      ...
      <tbody>
        <tr v-for="item in foods" :key="item.id">
          ...
          <td>
            <img
              :src="item.coverImage || 'https://via.placeholder.com/60x40'"
              alt=""
              style="
                width: 60px;
                height: 40px;
          ...
          <td>
            ...
            <small class="text-muted" v-if="item.description"
              ...
              }}{{ item.description.length > 30 ? "..." : "" }}</small
            >
          </td>
          <td>{{ item.province || "-" }}</td>
          <td>{{ item.city || "-" }}</td>
          <td>
            <span
              class="badge"
              :class="item.status === 1 ? 'bg-success' : 'bg-warning'"
            >
          ...
          <td class="action-btns food-actions">
            <button class="btn btn-info btn-sm" @click="viewDetail(item)">
              查看
            </button>
            <button
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">
            <i class="bi bi-info-circle text-info"></i> 美食详情
          </h5>
          <button
            type="button"
            class="btn-close"
          ...
          <div class="row">
            ...
            <div :class="detailData.coverImage ? 'col-md-8' : 'col-12'">
              ...
              <p class="text-muted mb-2">
                <i class="bi bi-geo-alt"></i> {{ detailData.province }} ·
                {{ detailData.city || "未知城市" }}
              </p>
              <div class="row mb-3">
                <div class="col-4">
                  ...
                  <span
                    class="badge"
                    :class="
                      detailData.status === 1 ? 'bg-success' : 'bg-warning'
                    "
                  >
                ...
                <div class="col-4">
                  ...
                  <div class="small">
                    {{
                      detailData.userId ? "用户" + detailData.userId : "管理员"
                    }}
                  </div>
                </div>
                <div class="col-4">
              ...
              <div v-if="detailData.address" class="mb-2">
                <span class="badge bg-info"
                  ><i class="bi bi-pin-map"></i> {{ detailData.address }}</span
                >
              </div>
            </div>
          </div>
...
const handleAudit = async (id, status) => {
  const msg =
    status === 1 ? "确定审核通过该美食吗？" : "确定取消审核该美食吗？";
  if (confirm(msg)) {
    try {
